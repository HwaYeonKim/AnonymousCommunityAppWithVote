...
exports.showQlist = function (req, res) {
	...
	//		{ $skip: 0}
		...
		showQlistFun(user_id,togender, str1, output1, page, res);
	} else if (category == 2) {
		//best
		var str2 = {Q_TOGENDER: togender};
	...
	//		{ $skip: 0}
		...
		showQlistFun(user_id,togender, str2, output2, page, res);
	...
	//		{ $skip: 0}
		...
		showQlistFun(user_id, togender,str, output, page, res);
	} else {
		var fail = {
			"RESULT ": "FAIL",
			"RESULT_MSG": 'error'
...
function showQlistFun(user_id,togender, str, aggreObj, page, res) {
				...
				async.waterfall([
								...
								function(data, callback){
									userCollection.findOne({"USER_ID" : user_id},{ _id:0 }, function(err, obj){
										if(err) {
											logger.error('error', '글 리스트 에러 : ', err);
										res.sendStatus(500);
									}
										if(obj == null){
											var fail = {
	...
	quesCollection.aggregate(output1, function(err,doc){
		...
		async.each(doc, function(data, callback1){
				async.waterfall([
						...
						if(err) {logger.error('error', '글 리스트 에러 : ', err);
						res.sendStatus(err);
					}
						callback1();
					})
