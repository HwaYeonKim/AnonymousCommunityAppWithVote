...
*/
	...
	form.parse(req, function (err, fields, files) {
		...
		if (err) {
			logger.error('error', '글 등록 에러 : ', err);
      ...
      var imageCount = parseInt(fields['q_imageCount']);
      var q_photo1, q_photo2;
      if(imageCount == 0){
      	q_photo1 = fields['q_photo1'];
      	q_photo2 = fields['q_photo2'];
      }else if(imageCount == 1){
      	q_photo1 = files['q_photo1'];
      	q_photo2 = fields['q_photo2'];
      }else {
      	q_photo1 = files['q_photo1'];
      	q_photo2 = files['q_photo2'];
      }
			// var q_photo1 = files.q_photo1;
			// var q_photo2 = files.q_photo2;
			console.log(q_photo2);
...
exports.updateQuestion = function (req, res){
	...
	async.waterfall([
			...
			async.series(seriesArray(q_photo1, q_photo2, user_id,results)
				,function (err, results) {
					if (err) {
						logger.error('error', '글 수정 에러 : ', err);
						console.log('Error', err);
						res.sendStatus(500); //Internal Server Error!
					}
					else{
...
exports.deleteQuestion = function (req, res){
	...
	quesCollection.findOne({_id : id}, { Q_COMM : 1, Q_PHOTO1 : 1, Q_PHOTO2 : 1, Q_THUMBNAIL1 : 1, Q_THUMBNAIL2 : 1 },
		function(err, result){
		if(err){
			logger.error('error', '글 삭제 에러 : ', err);
			res.sendStatus(500);
		}
		console.log(result);
		if(result == null){
